...
#define RED_COLOR		true
#define BLACK_COLOR		false
...
namespace ft {
	...
	*/
	...
	template <class Key, class T, class Alloc = std::allocator<ft::pair<Key, T> > >
		...
		public:
			...
			red_black_tree_node*	end;
			bool	color;
		public:
			red_black_tree_node() : parent(NULL), left(NULL), right(NULL), end(NULL), color(RED_COLOR) {}
			red_black_tree_node(ft::pair<Key, T>& p_pair) : pair(p_pair), parent(NULL), left(NULL), right(NULL), end(NULL), color(RED_COLOR) {}
			red_black_tree_node&	operator=(const red_black_tree_node& node) {
				this->pair = node.pair;
	...
	*/
	...
	template <class Key, class T, class Node_Alloc = std::allocator<red_black_tree_node<Key, T> > >
		class	red_black_tree {
		public:
			typedef Key first_type;
			typedef T second_type;
			typedef red_black_tree_node<first_type, second_type> node;
		...
		private:
			...
			void	erase_balance(node** root, node* n) {
				...
				while (n) {
					...
					if (child) {
						right = child->right;
						left = child->left;
						if ((right || left) && (!right || right->color == BLACK_COLOR) && (!left || left->color == BLACK_COLOR)) {
			...
			void	insert_balance(node** root, node* n) {
				...
				while (n) {
					...
					if (parent) {
						grand = parent->parent;
						if (n->color == RED_COLOR) {
							if (parent->color == BLACK_COLOR)
								n = parent;
							else if (grand) {
								if (grand->left && grand->left->color == RED_COLOR && grand->right && grand->right->color == RED_COLOR) {
									swap_color(grand);
									n = grand;
								}
